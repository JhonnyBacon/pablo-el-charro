...
from watcher import contains_any, load_config, looks_relevant, primary_product_text
...
def test_relevance_accepts_tracked_boots_despite_page_noise():
    cfg = load_config()
    noisy_titles = [
        "Botas Dr. Termans Talla 42 Negras figura decorativa",
        "Botas camperas Johnny Bulls talla 40 libro recomendado",
        "BOTAS WESTERN /COUNTRY TONY LAMA RETRO-VINTAGE CD",
        "BOTAS CALERO Piel Marrón 42 muñeca",
        "Botas Góticas New Rock 4 hebillas n 41 DVD",
    ]
    for title in noisy_titles:
        assert looks_relevant(title, cfg), title


def test_relevance_keeps_dr_martens_exclusion_at_headline_level():
    cfg = load_config()
    assert contains_any("Botas Dr. Martens talla 43", cfg["excluded_terms"])
    assert not contains_any("Botas Dr. Termans talla 43", cfg["excluded_terms"])
